<x-layouts::app>
    <div class="space-y-5">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-semibold text-xl text-text dark:text-text leading-tight">{{ __('Create Admin Balance') }}</h2>
                <p class="text-xs text-text/50">{{ __('Record admin charges from activated plans') }}</p>
            </div>
        </div>

        <div class="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800 p-5 shadow-sm">
            <form method="post" action="{{ route('admin.balances.store') }}" class="space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="number" name="amount" id="amount" value="{{ old('amount') }}" required min="0" step="0.01" class="w-full rounded-xl border border-gray-200 dark:border-neutral-600 bg-white dark:bg-neutral-800 px-3.5 py-2 text-sm text-text placeholder-text/30 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors">
                    </div>
                    <div>
                        ...
                        <select name="plan_id" id="plan_id" class="w-full rounded-xl border border-gray-200 dark:border-neutral-600 bg-white dark:bg-neutral-800 px-3.5 py-2 text-sm text-text placeholder-text/30 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors">
                            ...
                            @foreach($plans as $plan)
                                <option value="{{ $plan->id }}" {{ old('plan_id') == $plan->id ? 'selected' : '' }}>{{ $plan->name }} (₦{{ $plan->price }})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" required class="w-full rounded-xl border border-gray-200 dark:border-neutral-600 bg-white dark:bg-neutral-800 px-3.5 py-2 text-sm text-text placeholder-text/30 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors" placeholder="{{ __('Describe this balance entry') }}">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-3 border-t border-gray-100 dark:border-neutral-800">
                    ...
                    <button type="submit" class="px-5 py-2 rounded-xl bg-gradient-to-r from-primary to-secondary text-white text-sm font-semibold shadow-md shadow-primary/20 hover:from-primary/90 hover:to-secondary/90 transition-all">{{ __('Create Balance') }}</button>
                </div>
            </form>
        </div>
    </div>
