<x-layouts::app>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-text/50">Select a plan to activate</p>
            </div>
        </div>
    </x-slot>
    ...
    @php
        ...
        $isMaxPlan = $hasPlan && $currentPlan->sort_order >= $plans->max('sort_order');
    ...
    <div class="space-y-5 pb-8" x-data="{ selectedPlan: null }">
        {{-- Current Balance --}}
        ...
        <div class="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800 p-5 shadow-sm">
            <div class="flex items-center gap-2 mb-4">
                ...
                <h3 class="text-sm font-semibold text-text">Select a plan</h3>
            </div>
            ...
            <div class="grid grid-cols-2 gap-2 mb-4">
                @foreach($plans as $plan)
                    @php
                        $isCurrentPlan = $hasPlan && $currentPlan->id === $plan->id;
                        $isDowngrade = $hasPlan && !$isCurrentPlan && $plan->sort_order < $currentPlan->sort_order;
                        $isDisabled = $isDowngrade || $isCurrentPlan || $isMaxPlan;
                        $badgeColor = $planBadgeColors[$loop->index % count($planBadgeColors)];
                    @endphp
                    <button
                        type="button"
                        @if($isDisabled) disabled @endif
                        x-on:click="@unless($isDisabled) selectedPlan = { id: '{{ $plan->id }}', name: '{{ $plan->name }}', price: {{ $plan->price }} } @endunless"
                        class="relative py-3 rounded-xl border text-sm font-semibold transition-all flex flex-col items-center gap-1"
                        :class="@if($isDisabled)
                            'border-gray-200 dark:border-neutral-700 text-text/30 cursor-not-allowed opacity-50'
                        @else
                            'cursor-pointer'
                        @endif + (selectedPlan && selectedPlan.id === '{{ $plan->id }}' ? 'bg-primary/10 border-primary text-primary shadow-sm' : (!{{ $isDisabled }} ? 'border-gray-200 dark:border-neutral-700 text-text/60 hover:border-primary/30' : ''))"
                    >
                        <span class="text-xs font-bold" style="font-family: 'DM Serif Display', Georgia, serif;">₦{{ number_format($plan->price) }}</span>
                        <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-semibold border {{ $badgeColor }}">
                            {{ $plan->name }}
                        </span>
                        @if($isCurrentPlan)
                            <span class="text-[8px] text-text/30">Current plan</span>
                        @elseif($isDowngrade)
                            <span class="text-[8px] text-text/30">Can't downgrade</span>
                        @elseif($isMaxPlan)
                            <span class="text-[8px] text-text/30">Max plan</span>
                        @endif
                    </button>
                @endforeach
            </div>
            ...
            <form method="POST" action="{{ route('wallet.fund') }}">
                @csrf
                <input type="hidden" name="plan_id" :value="selectedPlan ? selectedPlan.id : ''">
                <div class="relative mb-4">
                    <span class="absolute left-4 top-1/2 -translate-y-1/2 text-text/40 font-semibold">₦</span>
                    <input
                        type="text"
                        name="amount"
                        :value="selectedPlan ? Number(selectedPlan.price).toLocaleString() : ''"
                        readonly
                        placeholder="Select a plan above"
                        required
                        class="w-full pl-10 pr-4 py-3.5 rounded-xl border border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-800 text-text font-bold text-xl focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all cursor-default"
                ...
                <button
                    ...
                    :class="!selectedPlan ? 'opacity-50 cursor-not-allowed' : ''"
                >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z"/></svg>
                    <span x-text="selectedPlan ? 'Fund Wallet — ₦' + Number(selectedPlan.price).toLocaleString() : 'Select a plan'">Select a plan</span>
                </button>
            </form>
        </div>
        ...
        <div class="bg-primary/5 dark:bg-primary/10 border border-primary/10 rounded-xl p-4">
            <div class="flex items-start gap-3">
                ...
                <div>
                    @if($isMaxPlan)
                        <p class="text-xs font-semibold text-primary mb-0.5">YOU'RE ON THE MAX PLAN</p>
                        <p class="text-[11px] text-text/40">You've reached the highest plan. Enjoy all premium features and start earning!</p>
                    @else
                        <p class="text-xs font-semibold text-primary mb-0.5">ACTIVATE A PLAN</p>
                        <p class="text-[11px] text-text/40">Select a plan above to unlock Voice Earn and Word Game features. You can only upgrade, not downgrade.</p>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div>
            <div class="flex items-center justify-between mb-3">
                ...
                <a href="{{ route('transactions.index') }}" class="text-xs font-medium text-primary hover:text-primary/80">View all</a>
            </div>
